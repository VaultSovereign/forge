...
async function findTemplate(projectRoot: string, keyword: string): Promise<{ tpl: Template, path: string }> {
  ...
  for (const family of families) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        console.error('[dispatcher] Failed to load template', absolute, err instanceof Error ? err.message : err);
      }
    }
  }
  return entries;
...
No additional commentary.`;
  ...
  if (ctx.profile && ctx.profile.voice && ctx.profile.voice.toLowerCase().includes("executive")) {
    usr = `Provide a concise executive summary first (<= 8 lines), followed by full details.\n\n` + usr;
  }
...
export async function runKeyword(opts: { projectRoot: string, keyword: string, profileName?: string|null, flags?: any, notes?: string }) {
  ...
  const requestedModel = typeof args.model === "string" && args.model.trim() ? args.model.trim() : defaultModelName;
  ...
  if ((args.output_format || "").toLowerCase() === "json") {
    ...
    if (!res.ok) {
      ...
      if (!res2.ok) {
        ok = false;
        output = { error: "Schema validation failed after repair", first_errors: res.errors, repair_attempt: repair };
      } else {
        output = res2.value;
      }
    } else {
  ...
  if (ledgerEligibleKeywords.has(tpl.keyword)) {
    try {
      ...
      console.error("[forge] reality ledger append failed:", err instanceof Error ? err.message : err);
    }
  }
