export type Risk = { title: string; severity: 'P0'|'P1'|'P2'|'P3'; why: string; where: string; fix: string; };
export type Opportunity = { title: string; value: string; effort: 'S'|'M'|'L'; steps: string[]; };
...
function toEvidenceItem(value: any): Evidence {
  if (value && typeof value === 'object') {
    ...
    const lines = typeof value.lines === 'string'
      ? value.lines
...
export function coerce(result: any) {
  ...
  result.modules = result.modules.map((module: any) => {
    ...
    mod.tests = {
      ...
      coverage: typeof tests.coverage === 'string' || typeof tests.coverage === 'number'
  ...
  result.top_risks = result.top_risks.map((risk: any): Risk => ({
  ...
  result.top_opportunities = result.top_opportunities.map((opp: any): Opportunity => ({
