...
export function createProviderConfig(): ProviderConfig {
  ...
  if (ollamaHost) {
    ...
    if (typeof fetch !== "function") {
      throw new Error("Global fetch API is required for Ollama provider (Node 18+). Set OLLAMA_HOST or upgrade runtime.");
    }
    ...
    return {
      ...
      async chat(model, system, user) {
        ...
        const msg = data?.message?.content ?? data?.messages?.map((m: any) => m?.content).join("\n");
        return String(msg ?? "");
      }
    };
  }
  ...
  if (!openaiKey) {
    throw new Error("No provider configured. Set OLLAMA_HOST, OPENROUTER_API_KEY, or OPENAI_API_KEY.");
  }
