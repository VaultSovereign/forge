# ⚔️ Forge Command L**VaultMesh Forge** — Earth's Civilization Ledger; sovereign prompt orchestration for compliance and cybersecurity.
...
<br/>
→ <a href="docs/OPERATIONS_RUNBOOK.md">Operations Runbook</a> Starter Repo (Blueprint v1)
...
- <a href="docs/README_RUN_MODES.md">Run Modes</a> — how to run locally (single-port, duo/HMR, smoke)
...
Security posture is reinforced with **OIDC/JWT** verification (JWKS), a **declarative RBAC** matrix, security headers/CSP, and a **dev bypass** for local development. Observability includes `/metrics` and status probes. Testing/CI are adequate but uneven across domains. **Net:** Forge is ready for controlled pilots with guardrails; production rollout is near once a handful of security, observability, and test gaps are closed.
...
## Single-Port Preview (recommended for local development)
...
Env summary
• AUTH_DEV_BYPASS=1 – local dev bypass (dev only)
